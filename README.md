...
- **The gap** — no official or established M3 Expressive implementation exists for the web (see [Material Design 3 for Web](https://m3.material.io/develop/web)). Every component here is audited against the [m3.material.io](https://m3.material.io) spec; deviations are documented, not hidden (`audit/`).
...
- **Ownable like shadcn, versioned like a package** — consume the [npm package](https://www.npmjs.com/package/m3-expressive-react) for semver updates, or copy the component source into your repo shadcn-style via the registry (see [the release plan](docs/open-source-release-plan.md) for the current status).
...
The agent handbook is served at `/llms.txt` on the running site. Every component's `exampleCode` is guaranteed runnable as-is.
...
| `audit/` | Per-family M3 spec audits: spec vs implementation vs documented deviations |
...
[MIT](LICENSE) — open source, open code. Built by [Medianeth](https://medianeth.com).
