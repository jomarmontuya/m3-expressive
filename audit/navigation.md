...
6. **Limitations:** underline width is the spec-approximate `w-1/3` centered (exact spec = label text width; needs per-tab text measurement — deferred); `aria-controls`/tabpanel linkage is left to consumers since the component renders only the tab strip.
