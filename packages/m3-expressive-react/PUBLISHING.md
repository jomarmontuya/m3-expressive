...
2. Node ESM sanity: import `./dist/index.js` and `./dist/theme-builder.js`; confirm `Button`, `m3Registry`, and `generateScheme` are functions/objects with no resolution error.
3. `npm pack --dry-run`; confirm dist/ + README + LICENSE + NOTICE, no stray files
...
5. From a CLEAN checkout after the package build: `cd packages/m3-expressive-react && npm publish --access public` (2FA OTP when prompted)
