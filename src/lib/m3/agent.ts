import { m3Registry } from "./registry";
import { m3Themes } from "./themes";
...
export const agentManifest = {
  ...
  forAgents: {
    ...
    protocol: [
      ...
      `5. Preferred for MCP clients: run mini-services/mcp-server and use ${MCP_TOOL_NAMES.join(", ")}.`,
    ...
    mcpHttp: {
      ...
      note: "Use the direct URL from localhost:3000. The relative gateway URL is for preview hosts that support XTransformPort. Start the service with `cd mini-services/mcp-server && bun run dev`.",
    },
    mcpServer: {
      name: "m3-expressive",
      transport: "stdio",
...
export function buildAgentHandbook(): string {
  ...
  lines.push(
    `- MCP server: stdio at \`mini-services/mcp-server\` with ${MCP_TOOL_NAMES.length} tools (${MCP_TOOL_NAMES.join(", ")}). Config: \`{"command":"bun","args":["run","--cwd","<abs>/mini-services/mcp-server","start"]}\`.`
  );
  lines.push(
    "- MCP over streamable HTTP: `POST http://localhost:3210/mcp`. Start it with `cd mini-services/mcp-server && bun run dev`."
  );
  lines.push(
