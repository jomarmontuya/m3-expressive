...
Task ID: 15-kickoff
Agent: orchestrator (Z.ai Code)
Task: Base UI migration kickoff — dependency install + shared conversion contract (user request: "use base-UI for each component instead of reinventing all components from scratch… fan out sub agents")

## Setup done
- Installed `@base-ui-components/react@1.0.0-rc.0` (bun add, 3 packages).
- Verified available primitives (node_modules/@base-ui-components/react): accordion, alert-dialog, autocomplete, avatar, button, checkbox, checkbox-group, collapsible, combobox, context-menu, dialog, field, fieldset, form, input, menu, menubar, meter, navigation-menu, number-field, popover, preview-card, progress, radio, radio-group, scroll-area, select, separator, slider, switch, tabs, toast, toggle, toggle-group, toolbar, tooltip, use-render. NO date-picker, NO carousel → those stay custom.
- Import pattern (verified in .d.ts): namespace parts — `import { Menu } from "@base-ui-components/react/menu"` then `<Menu.Root>` `<Menu.Trigger>` `<Menu.Portal>` `<Menu.Positioner>` `<Menu.Popup>` `<Menu.Item>`. Toast: `Toast.Provider/Viewport/Root/Title/Description/Close/Action` + `useToastManager()`/`createToastManager()`.

## SHARED CONVERSION CONTRACT (all 15-x agents MUST follow)
1. Convert ONLY your assigned files in `src/components/m3/`.
2. PRESERVE EXACTLY: filename; every named export (component + the `export { xxxMeta } from "@/lib/m3/meta"` line at the bottom); public prop names/types/defaults (adding optional props is OK; removing/renaming is NOT); `'use client'` as line 1.
3. BEHAVIOR: replace hand-rolled focus traps, roving tabindex, keyboard handling, ARIA wiring, Escape/outside-click handling, portals with the matching Base UI headless parts. Base UI owns behavior + a11y; WE own the visuals.
4. VISUALS UNCHANGED: keep the M3 design system — Tailwind `m3-*` token classes, `md-*` type classes, `.m3-state`/`.m3-focus`, springs/easings from `@/lib/m3/tokens`, `MaterialSymbol`, `Ripple` (where it exists today), shapeMorph, border-radius scale. Style Base UI parts via `className` props (they accept them); NEVER rely on Base UI's default look (it has none — it's headless).
5. Portals: Base UI popups render through portals. Our CSS vars live on :root/.dark/[data-theme] so token styling still works at body level — do NOT add extra providers.
6. Framer-motion: keep for entrance/expressive motion where it exists today; do not double-animate Base UI transitions (its parts support CSS transition end-state detection via inline `--transition-duration`… if a popup needs open/close animation, use Base UI's `[data-open]`/`[data-closed]` states with CSS or keep motion for content-only).
7. NO matching primitive → keep the current hand-rolled logic and add a one-line comment `// No Base UI primitive for <thing> in v1.0.0-rc.0 — custom implementation retained`.
8. BEFORE using a primitive, read its parts: `node_modules/@base-ui-components/react/<name>/index.parts.d.ts` (and the part's .d.ts for props). Do not guess prop names.
9. FORBIDDEN: touching meta.ts / registry.ts / themes / tokens / demos/* / playground-specs / showcase files / index.ts / other agents' component files; running dev server; running vr scripts; installing packages; editing package.json.
10. VERIFY (scoped): `cd /home/z/my-project && bunx tsc --noEmit 2>&1 | grep "src/components/m3/<YourFiles>"` — fix ALL errors in YOUR files; IGNORE errors in files outside your assignment (7 agents are editing concurrently). Also grep your files for leftover TODOs.
11. Append your section to this worklog (Task ID 15-x, three-part template).
12. Report back: per-file → primitives used / fallback rationale; tsc status for your files; any public-API additions.

## Assignment map (41 components / 7 agents)
- 15-a Buttons: Button, IconButton, FAB, ExtendedFab, ButtonGroup → Base UI `button` (semantic <button>, useButton where helpful)
- 15-b Toggles/selection: SegmentedButton, Chip, Checkbox, Radio, Switch, Slider → `toggle-group`, `toggle`, `checkbox`, `radio-group`/`radio`, `switch`, `slider`
- 15-c Menus/popups: Menu, FabMenu, SplitButton, Tooltip, Autocomplete → `menu`, `tooltip`, `autocomplete` (parts: Root/Input/Popup/List/Item/Group/GroupLabel)
- 15-d Overlays: Dialog, BottomSheet, SideSheet, Snackbar, Banner → `dialog` (+alert-dialog), `toast` (Snackbar), `collapsible` (Banner) — sheets = Dialog with custom Positioner side/animation
- 15-e Inputs: TextField, SearchBar, SearchView, DatePicker, TimePicker → `field`+`input` (TextField/SearchBar), SearchView = input+dialog popover, DatePicker/TimePicker keep custom pickers (no Base UI date primitive) but may adopt `field` wrappers where it helps labels/descriptions
- 15-f Navigation/containers: Tabs, Toolbar, TopAppBar, BottomAppBar, NavigationBar, NavigationDrawer, NavigationRail, Divider → `tabs` (Tabs), `toolbar` (Toolbar + app bars where semantics fit), `separator` (Divider)
- 15-g Feedback/display: LinearProgress, CircularProgress, LoadingIndicator, Badge, Card, List, Carousel → `progress` (Linear), Badge→`badge` if it helps else custom, CircularProgress/LoadingIndicator/Carousel/Card/List stay custom (no primitive) — only adopt where real value

---

Task ID: 15-a
Agent: general-purpose
Task: Buttons → Base UI — convert Button, IconButton, FAB, ExtendedFab, ButtonGroup onto the Base UI `button` primitive (keep M3 Expressive visuals, public API byte-compatible)

Work Log:
- Read 15-kickoff contract. Read `node_modules/@base-ui-components/react/button/` (NOTE: this package has no `index.parts.d.ts`; it ships a single `Button` part via `index.d.ts` → `Button.d.ts`). Verified from `Button.js`/`useButton.js`/`useFocusableWhenDisabled.js`/`utils/useRenderElement.js`/`merge-props/mergeProps.js`:
  - `Button` renders a native `<button>`, sets `type="button"` by default (user-passed `type` still wins), guards `onClick`/`onPointerDown` while disabled, sets the DOM `disabled` attr (native, non-composite, `focusableWhenDisabled=false` default) and merges `className`/`style`/event handlers properly.
  - `render` accepts a ReactElement: Base UI clones it and merges its own props (handlers composed rightmost-first, classNames concatenated, refs merged). So `render={<motion.button ...motionProps />}` keeps framer-motion in charge of whileTap/whileHover/shape-morph while BaseButton owns semantics — no TS conflict spreading DOM props onto motion (verified: element-form `render` typechecks; raw `{...domProps}` spread into `render` fn does NOT because framer redefines `onAnimationStart` etc.).
- Temporarily created `scratch-15a.tsx` (project root) to empirically confirm the 3 viable render-form typings + `forwardRef<HTMLButtonElement> → BaseButton ref (Ref<HTMLElement>)` assignability via @types/react 19 bivarianceHack; deleted it after.
- Converted the 5 files to a common layering: BaseButton = semantic/behavior layer (props: ref, disabled, type, aria-*, data-*, wrapped handlers, className, style, {...props}); framer-motion `motion.button` = visual layer via `render` (whileTap/whileHover/animate/transition/onHoverStart/onHoverEnd + children: Ripple, spinner, icons). All M3 token classes, `.m3-state`/`.m3-focus`, touch-target ::before extensions, elevation classes, springs (`fastVisual`, `expressive`, `expressiveEffects`, `fastSpatial`, `defaultSpatial`), `shapeMorph`, size/variant/shape/disabled/selected style tables and the meta re-export lines kept verbatim.
- ButtonGroup: container kept a plain `role="group"` div (aria-label plumbs via `...props` spread) with the mandated comment `// No Base UI primitive for a connected button group in v1.0.0-rc.0 — custom container retained` (placed in the doc block, phrased verbatim); the per-segment buttons were moved onto the same BaseButton+motion layering (public API — `buttons`/`selection`/`value`/`onValueChange`/`variableWidths`/`size`/`disabled` + all type exports — untouched, so demos/PropsPlayground keep working).
- Verification: scoped `bunx tsc --noEmit | grep "src/components/m3/(Button|IconButton|FAB|ExtendedFab|ButtonGroup)\.tsx"` → zero errors. Full tsc sweep showed only 4 pre-existing errors in `skills/` and `examples/` (outside src, not touched). No TODO/FIXME left in the 5 files.

Stage Summary:
- Button.tsx — Base UI part: `Button` (only part in `@base-ui-components/react/button` v1.0.0-rc.0) composed with `render={<motion.button/>}`. Base UI owns native `<button>` + `type="button"` default + disabled click/pointer guarding; framer-motion owns press scale (`springs.fastVisual`) and borderRadius shape morph (`springs.expressiveEffects` + `shapeMorph`); keyboard press→morph mirroring (Space/Enter) kept in wrapped onKeyDown/onKeyUp/onBlur. Props `variant/size/shape/icon/trailingIcon/loading/fullWidth` + all defaults unchanged; `aria-busy`, `data-pressed` preserved; `export { buttonMeta }` intact. Relying on Base UI's `type="button"` default instead of a hardcoded attr (user-passed `type` still wins — same as before).
- IconButton.tsx — same `Button` part + motion `render`. Toggle logic (controlled/uncontrolled `selected`/`onSelectedChange`, `aria-pressed`), keyed-remount spring pop, variant/selected/disabled token classes, touch targets: all kept. `type={type ?? "button"}` kept. Base UI additionally guards onClick while disabled (previously only the native attr did).
- FAB.tsx — same `Button` part + motion `render` (hover lift 1.03/tap 0.94 with `springs.expressive`; `onHoverStart/End` stay on the motion element — they are framer-specific props with custom signatures). `fabColorStyles` + `FabColor` exports kept (imported by ExtendedFab and FabMenu — another agent's file). Size/shape/elevation/lowered/disabled tokens unchanged.
- ExtendedFab.tsx — same `Button` part + motion `render`; anatomy (56dp, rounded-2xl, 24dp icon, label-large) and elevation model unchanged; meta re-export intact.
- ButtonGroup.tsx — no group primitive in Base UI v1.0.0-rc.0 → container kept custom (plain div, `role="group"`, aria-label via props spread, mandated comment added). Segments converted to the shared BaseButton+motion layering; single/multiple/none selection logic, controlled `value`, variableWidths flexGrow spring, per-item onClick composition: unchanged. Public API + all type exports (`ButtonGroupItem/Variant/Selection/Size`) unchanged.
- API additions: none. API changes: none. Known micro-nuance: Base UI merges `style` per-key (our token style wins conflicts, user's extra keys survive) instead of full replacement — demos don't pass `style` to these components, so no observable change.
- Risks for integration: (1) Base UI composes handlers (ours run before Base UI's internal guards) — consumers calling `preventDefault` in onClick still work; (2) framer-motion still receives DOM handlers via cloneElement merge — the props interfaces keep the same `Omit<...onAnimationStart/onDrag*>` guards, so conflicting handlers can't enter through our API; (3) `@base-ui-components/react/button` exposes no other parts (no Root sub-object), so the import is `import { Button as BaseButton } from "@base-ui-components/react/button"`.

---

Task ID: 15-g
Agent: general-purpose
Task: Base UI migration — feedback/display group (LinearProgress, CircularProgress, LoadingIndicator, Badge, Card, List, Carousel)

Work Log:
- Read the 15-kickoff contract; verified primitives in node_modules BEFORE use: `progress` exists (index.parts.d.ts: Root/Track/Indicator/Value/Label, `value: number | null`, status data-attributes `data-indeterminate/-progressing/-complete`; Indicator gets inline `inset-inline-start: 0` + percentage `width` when determinate, nothing when null). **`badge` does NOT exist in @base-ui-components/react@1.0.0-rc.0** (no badge/ folder, not in index.d.ts exports map — added upstream only after rc.0).
- LinearProgress.tsx → converted to Base UI Progress: `Progress.Root` (role="progressbar", aria-valuemin/max/now, locale-independent `getAriaValueText`), `Progress.Label` (wires aria-labelledby; replaces the hand-rolled label span), `Progress.Value` (aria-hidden % display; deterministic `Math.round` text via children render fn to avoid SSR/client Intl drift), `Progress.Track` (visual rail), `Progress.Indicator` (value-sliced bar / indeterminate clip container; Base UI owns the inline % width). Flat determinate bar animates via CSS `transition: width 400ms emphasized` (end-state approximation of the old spring; no double-animation of Base UI-owned width). Indeterminate strips + full wavy M3E treatment unchanged inside the parts. No buffer/range styling in Base UI rc or in our API — noted in the doc comment.
- CircularProgress.tsx → NO radial primitive (verified) — custom SVG ring + rotation retained. Wrapped each branch in `Progress.Root` (value / null, aria-label, deterministic getAriaValueText, `inline-block shrink-0` on Root) so the progressbar role/ARIA + status attributes come from Base UI; the ring is now an `aria-hidden` motion.svg inside (avoids double announcement). Public API unchanged.
- LoadingIndicator.tsx → no primitive exists — kept custom, added the retention comment.
- Badge.tsx → `badge` primitive is absent from the installed v1.0.0-rc.0 (task brief assumed otherwise; verified against package exports) → kept custom with retention comment; all props (value/count, showDot, anchor children, color, max 99+, disabled) untouched.
- Card.tsx → added `// Presentational container — no Base UI primitive needed` comment; zero DOM/API changes.
- List.tsx / Carousel.tsx → kept custom with retention comments; Carousel internals untouched (2 comment lines only).
- Scoped verify: `bunx tsc --noEmit | grep m3/(my 7 files)` → 0 errors. No TODOs left. All 7 files: 'use client' line 1, named exports + meta re-export lines intact, props unchanged.

Stage Summary:
- 7/7 files done. Base UI Progress adopted where it gives real semantics (Linear fully; Circular for the role/ARIA wrapper only). Badge/Card/List/LoadingIndicator/Carousel retained custom — no matching primitive in 1.0.0-rc.0 (badge verified absent, despite brief).
- Integration risks for the orchestrator: (1) CircularProgress/LinearProgress DOM gained a `role="progressbar"` wrapper element / Base UI attribute set — visual parity preserved, but any consumer doing `svg > *`-style DOM queries on CircularProgress would need updating (none found in demos/playground/showcase). (2) LinearProgress determinate fill animates width via CSS (400ms emphasized) instead of the framer spring — end states identical, feel marginally different. (3) If the project bumps @base-ui-components/react past rc.0, Badge.tsx is the first candidate for a follow-up conversion.

---

Task ID: 15-f
Agent: general-purpose
Task: Base UI migration — navigation/containers group (Tabs, Toolbar, TopAppBar, BottomAppBar, NavigationBar, NavigationDrawer, NavigationRail, Divider)

Work Log:
- Read the 15-kickoff shared conversion contract + all 8 assigned files; read Base UI .d.ts parts before use (tabs/index.parts.d.ts, toolbar/index.parts.d.ts, dialog/index.parts.d.ts, separator/Separator.d.ts) and cross-checked runtime sources in esm/ (TabsRoot/TabsList/TabsTab, ToolbarRoot/ToolbarButton, DialogRoot/store/useOpenStateTransitions/useTransitionStatus, Separator, useRenderElement/mergeProps, useButton) — no prop names guessed.
- Tabs.tsx → FULL Base UI Tabs conversion: `import { Tabs as BaseTabs } from "@base-ui-components/react/tabs"` (aliased: module exports its own `Tabs`). BaseTabs.Root = controlled wrapper + onValueChange (DOM: same outer div); BaseTabs.List = role="tablist" scroller div (ref kept for overflow detection) with `activateOnFocus` (preserves our automatic-activation arrows) and default loopFocus=true (preserves wrap-around); BaseTabs.Tab per item (role="tab", aria-selected, roving tabindex, type="button" all owned by Base UI). Removed hand-rolled tabRefs + onTabKeyDown (ArrowLeft/Right/Home/End now Base UI's; verified CompositeRoot enableHomeAndEndKeys). Scroll arrows stay custom (Base UI Tabs has no scroller/arrows). INDICATOR DECISION: kept the framer-motion shared-layout overlay (layoutId + springs.expressive), NOT Tabs.Indicator — Base UI's indicator sizes to the whole tab via CSS transitions, while M3 primary needs the underline sized to the *measured label text* (ResizeObserver + document.fonts.ready) and both indicators must spring; overlay keeps the look pixel-identical. aria-controls stays absent because there are no panels (verified: mountedTabPanels empty → attribute omitted). All public props preserved (items/value/onChange/variant/fullWidth/className).
- Toolbar.tsx → Base UI Toolbar: `import { Toolbar as BaseToolbar }` (name clash). Dockable variant: the bar div IS BaseToolbar.Root (role="toolbar", aria-orientation) with identical style/className. Floating variant: the animated pill composes via Base UI `render={<motion.div …/>}` so the toolbar root *is* the framer-motion element (verified mergeProps semantics: Base UI's role/handlers/ref merge onto the motion element; initial/animate/transition untouched). All icon buttons are BaseToolbar.Button (roving tabindex + arrow keys across the pill, via context registration — verified ToolbarButton→CompositeItem, works as direct children). aria-pressed/aria-label/title/onClick pass straight through. Public API unchanged.
- TopAppBar.tsx → kept custom layout/scroll-elevation/collapse (no app-bar primitive). Required comment added verbatim. Actions row (when non-empty) wrapped in Toolbar.Root with Toolbar.Button action buttons (leading back button deliberately excluded — it sits outside the toolbar; empty `actions` keeps the original plain spacer div to avoid an empty role="toolbar"). Public API/DOM unchanged otherwise.
- BottomAppBar.tsx → same treatment: the 80dp bar div IS Toolbar.Root; navigation icon, action and trailing buttons are Toolbar.Buttons; the center FAB intentionally stays a motion.button outside the roving group (normal tab position). Required comment added verbatim. Public API/DOM unchanged.
- NavigationBar.tsx → kept custom with contract #7 comment (`// No Base UI primitive for a bottom navigation bar…` + note that navigation-menu is NOT bottom nav). Verified aria-current="page" on active destination (was already correct). No code-path changes.
- NavigationDrawer.tsx → modal variant migrated to Base UI Dialog (Dialog.Root/Portal/Backdrop/Popup); standard variant kept as inline panel with a comment (no dialog semantics apply). Base UI now owns Escape dismissal, scrim/outside-press dismissal (Backdrop registers the dismissal target), focus trap, focus restore on close, and body scroll lock — deleted all of that hand-rolled code (restoreFocusRef/Tab-trap/overflow-lock effect). Public API unchanged (items/value/onChange/variant/open/onClose/header/footer/fullHeight/className). Animation: M3 slide is a JS-driven physics spring (no CSS transition for Base UI's auto-unmount to await), so on close `onOpenChange` calls `details.preventUnmountOnClose()` and the dialog is imperatively unmounted via `actionsRef.unmount()` in the popup's framer-motion `onAnimationComplete` once the slide-out spring finishes (documented Base UI escape hatch; verified against useOpenStateTransitions/forceUnmount). Destination selects route through `actionsRef.close()` so every close shares one exit path. Scrim = Dialog.Backdrop render motion.div (`bg-m3-scrim/32`, defaultVisual spring); panel = Dialog.Popup render motion.nav (360dp, defaultSpatial spring), popup gets role="dialog"+tabIndex=-1 from Base UI, aria-label preserved.
- NavigationRail.tsx → kept custom with contract #7 comment; verified aria-current="page" on active destination. No code-path changes.
- Divider.tsx → swapped to Base UI Separator (`import { Separator } from "@base-ui-components/react/separator"`, single part). Separator owns role="separator" + aria-orientation (it always renders aria-orientation, matching the old hand-rolled output); inset/thickness/color/orientation/className logic untouched via className/style. No label prop exists in the public API (noted in docblock for a future labeled variant).
- Scope respected: only my 8 files touched; no meta/registry/demo/playground/index/package changes; no packages installed; no dev server/vr/lint.

Stage Summary:
- 8/8 files migrated. Primitives: tabs (Root/List/Tab), toolbar (Root/Button — also adopted by both app bars' action rows), dialog (Root/Portal/Backdrop/Popup — modal drawer only), separator (Divider). Fallbacks kept custom on purpose: Tabs scroll arrows (no scroller primitive), app-bar layout/scroll-elevation/collapse, NavigationBar/NavigationRail (no primitive; navigation-menu is a menu primitive, not nav), standard drawer panel (inline, no dialog semantics), and the Tabs selection indicator (framer-motion shared-layout overlay chosen over Tabs.Indicator to preserve label-width underline + M3 springs — decision documented in source).
- tsc: `bunx tsc --noEmit | grep "src/components/m3/(Tabs|Toolbar|TopAppBar|BottomAppBar|NavigationBar|NavigationDrawer|NavigationRail|Divider).tsx"` → zero errors for my files (repo-wide tsc at time of writing: only 4 pre-existing errors under examples/ and skills/, none in src/).
- Public-API additions: none (all prop names/types/defaults preserved; TopAppBar's AppBarIconButton gained an internal-only `toolbar` flag — not exported). Named exports + meta re-export lines intact; 'use client' line 1 everywhere.
- Behavior notes for reviewers: (1) drawer initial focus now lands on the first destination (Base UI default) instead of the panel itself — better SR announcement, same tab/Escape/scrim behavior; (2) app-bar/toolbar buttons now use roving tabindex + arrow keys (new keyboard capability, no visual change); (3) clicking the active tab no longer re-fires onChange (Base UI early-returns on already-active tabs) — externally invisible.
- Integration risks: (a) Base UI tabs/toolbar are RTL-direction aware via useDirection — if the app later wraps in a DirectionProvider, arrow keys flip accordingly (desired). (b) The drawer relies on preventUnmountOnClose + actionsRef.unmount — if a future refactor adds CSS transitions to the popup, Base UI's auto-unmount (disabled by preventUnmountingOnClose) stays disabled, so the manual unmount remains the single teardown path. (c) Toolbar floating render-prop: Base UI clones the motion.div; framer-motion upgrades to the render-prop element are safe under current merge semantics (verified in useRenderElement/mergeProps), but a major Base UI upgrade should re-verify mergeProps precedence. (d) Divider now always renders aria-orientation (identical to before) — no consumer impact.

---
Task ID: 15-final
Agent: orchestrator (Z.ai Code)
Task: Base UI migration completion — converted the 10 components left after agent infra failures, runtime-verified everything, refreshed VR

## Current project status
MIGRATION COMPLETE. All 41 registry components are now Base UI-backed or custom-by-design (documented). The two agent fan-out waves hit infra failures (context deadline ×5, server_shutdown ×5, rate-limit ×1); agents delivered 15-a/15-f/15-g fully and 15-b/15-d/15-e partially (with 4 mid-edit corrupted files) before dying. Orchestrator finished the remaining 10 components directly, fixed the corrupted files, and verified everything end-to-end.

## Completed this round (verification results)
- Fixed agent leftovers: Checkbox.tsx + Switch.tsx (Base UI namespace import collided with local export names → aliased BaseCheckbox/BaseSwitch), SegmentedButton.tsx (React defaultValue/defaultChecked type conflict with ToggleGroupProps → Omit in public type), SearchView.tsx (untyped motion-props bundle → HTMLMotionProps<"div">).
- Converted by orchestrator: Dialog (Root/Portal/Backdrop/Popup; focus trap+scroll lock+Escape+focus-restore from Base UI; dismissible filters escape-key/outside-press reasons; transform-free centering via inset-0+m-auto so framer owns the transform), Menu (Trigger render-prop composition replaces cloneElement; Positioner side/align; Item roving focus/typeahead; live-verified: expanded=true, ArrowDown roving focus, Enter closes), Tooltip (Provider 500/600ms MDC delays; hoverable popup for rich actions; custom caret kept), Slider (Root/Control/Track/Indicator/Thumb — CRITICAL lesson: Thumb children via normal children slot, NOT render element, or Base UI's hidden input[type=range] carrying focus/ARIA is dropped; live-verified: input[type=range] aria-valuenow=40→42 on ArrowRight), BottomSheet + SideSheet (Dialog-based; framer drag-to-dismiss + slide exits kept via preventUnmountOnClose + actionsRef.unmount), SplitButton (Menu.Trigger on the chevron segment), FabMenu (Menu.Root/Trigger/Item; cascade lives INSIDE a floating Positioner above the FAB — Menu.Popup REQUIRES Positioner, first in-flow attempt errored; live-verified: 3 items, Escape closes, focus restores to trigger), Autocomplete (Root mode="list" filters internally; Input/Trigger/Positioner/Popup/List/Item; live-verified: typing filters ["re"]→[React,Preact], "No matches" fallback renders; trade-off documented: Base UI owns popup mount so no exit animation).
- Snackbar (from dead agent 15-d2, verified complete): BaseToast.createToastManager + Provider/Viewport/useToastManager; fixed lint empty-interface → Pick type alias.
- Docs install command updated: `bun add @base-ui-components/react framer-motion clsx tailwind-merge` (4 packages now); header copy "3 runtime dependencies" → 4.
- Dev server had died (0 processes, likely from the session's server_shutdown) — restarted via setsid, healthy.
- Gates: tsc 0 src errors; lint 0 errors / 1 pre-existing warning; mobile 375px sw=375; dev.log clean; MCP healthy (untouched, 14 tools/41 components); **full VR refresh 41/41 → PASS identical 40 · minor 1 · changed 0 — the Base UI migration is pixel-equivalent**.
- webDevReview cron active (job 341240).

## Component ledger (41)
- On Base UI (31): Button, IconButton, FAB, ExtendedFab, ButtonGroup*, SegmentedButton, Chip*, Checkbox, Radio, Switch, Slider, Menu, FabMenu, SplitButton, Tooltip, Autocomplete, Dialog, BottomSheet, SideSheet, Snackbar, TextField*, SearchBar*, SearchView*, Tabs, Toolbar, TopAppBar*, BottomAppBar*, NavigationDrawer*, Divider, LinearProgress, CircularProgress (* = Base UI for the semantic layer, custom layout retained)
- Custom-by-design (10, each with a documented "no Base UI primitive in v1.0.0-rc.0" comment): Card, List, Carousel, LoadingIndicator, Badge (no badge module in rc.0), NavigationBar, NavigationRail, Banner (dismissible-only; Collapsible adds nothing), DatePicker, TimePicker (no date/time primitives)

## Unresolved issues / risks / next-phase priorities
- Base UI is 1.0.0-rc.0 — pin until 1.0 stable; revisit Badge (module arrives post-rc) and DatePicker/TimePicker when Base UI ships date primitives.
